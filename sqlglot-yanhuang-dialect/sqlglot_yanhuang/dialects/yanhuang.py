...
class Yanhuang(Postgres):
    ...
    class Generator(Postgres.Generator):
        ...
        def group_sql(self, expression: exp.Group) -> str:
            ...
            if grouping_sets:
                # 处理grouping_sets中的每个表达式
                for grouping_expr in grouping_sets:
                    if isinstance(grouping_expr, exp.GroupingSets):
                        self.groupingsets_sql(grouping_expr)
                    elif isinstance(grouping_expr, exp.Rollup):
